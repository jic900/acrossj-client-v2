...
import {
  ...
  IOthersInfoMessage,
  OthersInfoConfig
} from 'app/config/user/profile/eventrelated/othersinfo.config';
import { LocalStorageService } from 'app/shared/services/localstorage.service';
import { ProfileService } from 'app/features/user/services/profile.service';
import { IProfile, IOthers } from 'app/features/user/model/profile.model';

@Component({
  selector: 'aj-othersinfo',
  templateUrl: './othersinfo.component.html',
  styleUrls: ['./othersinfo.component.scss']
})

export class OthersInfoComponent implements AfterViewInit, OnDestroy {

  formData: IForm;
  formElements: IOthersInfo;
  messages: IOthersInfoMessage;
  formGroup: FormGroup;
  ...
  constructor(private profileService: ProfileService, private localStorageService: LocalStorageService) {
    this.formData = new OthersInfoConfig();
  ...
  ngAfterViewInit(): void {
    ...
    if (profileInStorage) {
      ...
      this.populateOthersInfo(profile.relevant ? profile.relevant.others : null);
    } else {
      this.loadOthersInfo();
    }
  }
  ...
  populateOthersInfo(others: IOthers): void {
    setTimeout(() => {
      if (others) {
        Object.keys(others).forEach((field) => {
          this.formGroup.get(field).setValue(others[field]);
        });
      }
    });
  }
  ...
  onSave(event: any): void {
    ...
    this.profileService.saveProfile({'relevant.others': this.formGroup.value})
      .subscribe(
        data => {
