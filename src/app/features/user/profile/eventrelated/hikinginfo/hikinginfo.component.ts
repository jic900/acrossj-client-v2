...
import {
  ...
  IHikingInfoMessage,
  HikingInfoConfig
} from 'app/config/user/profile/eventrelated/hikinginfo.config';
import { LocalStorageService } from 'app/shared/services/localstorage.service';
import { ProfileService } from 'app/features/user/services/profile.service';
import { IProfile, IHiking } from 'app/features/user/model/profile.model';

@Component({
  selector: 'aj-hikinginfo',
  templateUrl: './hikinginfo.component.html',
  styleUrls: ['./hikinginfo.component.scss']
})

export class HikingInfoComponent implements AfterViewInit, OnDestroy {

  formData: IForm;
  formElements: IHikingInfo;
  messages: IHikingInfoMessage;
  formGroup: FormGroup;
  ...
  constructor(private profileService: ProfileService, private localStorageService: LocalStorageService) {
    this.formData = new HikingInfoConfig();
    ...
    this.subscription = this.profileService.profileUpdated$.subscribe(
      (profile: IProfile) => this.populateHikingInfo(profile.relevant ? profile.relevant.hiking : null)
    );
  }
  ...
  ngAfterViewInit(): void {
    ...
    if (profileInStorage) {
      ...
      this.populateHikingInfo(profile.relevant ? profile.relevant.hiking : null);
    } else {
      this.loadHikingInfo();
    }
  }
  ...
  populateHikingInfo(hiking: IHiking): void {
    setTimeout(() => {
      if (hiking) {
        Object.keys(hiking).forEach((field) => {
          this.formGroup.get(field).setValue(hiking[field]);
        });
      }
    });
  }
  ...
  onSave(event: any): void {
    ...
    this.profileService.saveProfile({'relevant.hiking': this.formGroup.value})
      .subscribe(
        data => {
